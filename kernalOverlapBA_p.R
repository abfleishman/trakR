# For this function to work we need a data frame withSex, a unique tripID
# column, and lat/lon
tracks<-inner_join(MexHg,tracks,by="ID") %>% 
  dplyr::select(Latitude,Longitude,date,tripID,
                Species.x,Island.x,ID,final.Hg,Sex.x,BrStage.x,Mass) %>% 
  rename(Species=Species.x,Island=Island.x,Sex=Sex.x,BrStage=BrStage.x) %>% 
  mutate(ColSpSex=as.factor(paste(Island,Species,Sex,sep="_"))) %>% 
  rename(BirdNum=ID,ID=ColSpSex) %>% 
  filter(Island=="SJ")

# This is for making a grid.  we take the extent of the tracks SPDF and then
# buffer it and make a grid with a res of x
trj_temp<-SpatialPoints(cbind(tracks$Longitude,tracks$Latitude), proj4string = CRS("+proj=longlat +ellps=WGS84 +datum=WGS84 +no_defs"))
trj_temp<-spTransform(trj_temp,paste("+proj=laea +units=km +lon_0=", -113.244555, " +lat_0=", 31.015513	, sep=""))


# Make a grid for UD ----
# Resolution for UD grid in KM
res<-0.5
# Make a raster with the same extent as as your SPDF above with a buffer in the
# units that the spdf is in
e<-raster::extent(raster::buffer(trj_temp,width=res*50)) 

# Make a raster from the extent with a resolution in the units you have
cos.grid<-ascgen(as(e,"SpatialPoints"),cellsize = res)
...
# Run the function
overlap_sj_p<-OverlapBA_p(tracks, tripid="tripID", groupid = "Sex",
                          lon="Longitude",lat="Latitude",
                          colonyLon=-113.244555,colonyLat=31.015513,
                          its=10, h=5, ud.grid=cos.grid,Plot=F)
