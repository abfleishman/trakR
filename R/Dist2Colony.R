...
#' @export
Dist2Colony<-function(tracks,dataLat="lat",dataLon="lon",ColonyLat,ColonyLong){
  ...
  for(i in 1:length(tracks$Latitude)){ #this is a for loop
    ...
    Point2Colony[i]<-argosfilter::distance(lat1 = ColonyLat,lon1 = ColonyLong ,lat2 = tracks[[dataLat]][i],lon2 = tracks[[dataLon]][i])
